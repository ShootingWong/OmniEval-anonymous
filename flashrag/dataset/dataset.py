...
class Item:
    ...
    def __init__(self, item_dict):
        ...
        if self.golden_answers == []:
            self.golden_answers = item_dict.get("answer", [])
            
        self.choices = item_dict.get("choices", [])
        self.metadata = item_dict.get("metadata", {})
        self.output = item_dict.get("output", {})
        self.relevant_passage = item_dict.get("relevant_passage", {}) # self add
        self.relevant_node = item_dict.get("relevant_node", {})# self add
        self.data_quality = item_dict.get("data_quality", {})# self add
        
        self.round = item_dict.get("round", None) #self add for conversation
    ...
    def __getattr__(self, attr_name):
        if attr_name in ["id", "question", "golden_answers", "metadata", "output", "choices", "relevant_passage", "relevant_node"]:
    ...
    def to_dict(self):
        ...
        output = {
            ...
            "output": self.output,
            "relevant_passage": self.relevant_passage, # self add
            "relevant_node": self.relevant_node, # self add
        }
        if self.metadata != {}:
...
class Dataset:
    ...
    def __init__(self, config=None, dataset_path=None, data=None, sample_num=None, random_sample=False):
        self.config = config
        self.dataset_name = config["dataset_name"] if config is not None else None
        ...
        with open(dataset_path, "r", encoding="utf-8") as f:
            ...
            for line in f:
                ...
                if isinstance(item_dict, list):
                    ...
                    item = []
                    for i, idict in enumerate(item_dict):
                        idict['round'] = i
                        if 'id' not in idict:
                            idict['id'] = data_id
                        item.append(Item(idict))
                    data += item
                else:
                    if 'id' not in item_dict:
                        item_dict['id'] = data_id
                    item = Item(item_dict)
                    data.append(item)
                data_id += 1
    ...
    def question(self):
        if self.conversation:
            ...
            for i, item in enumerate(self.data):
                if item.round > 0:
                    pre_questions = [self.data[pre_i].question for pre_i in range(i-item.round, i)]
                else:
                    pre_questions = []
                all_questions = pre_questions + [item.question]
    ...
    def save(self, save_path):
        ...
        # print(f'=====> save_data[0] = {save_data[0]}')
