...
class BasicPipeline:
    ...
    def __init__(self, config, prompt_template=None, conv_prompt=None):
        self.config = config
...
class SequentialPipeline(BasicPipeline):
    def __init__(self, config, prompt_template=None, retriever=None, generator=None, conv_prompt=None):
        """
        inference stage:
            query -> pre-retrieval -> retriever -> post-retrieval -> generator
        """

        super().__init__(config, prompt_template, conv_prompt)
    ...
    def build_conv_input(self, input_querys):
        new_querys = []
        for querys in input_querys:
            querys_str = '\n'.join([f'{i}-round: {querys[i]}' for i in range(len(querys)-1)])

            new_querys.append(self.conv_prompt.format(querys_str, querys[-1]))
        return new_querys

    def naive_run(self, dataset, do_eval=True, pred_process_fun=None):
        # direct generation without RAG
        input_query = dataset.question
        if isinstance(input_query[0], list):
            input_query = self.build_conv_input(input_query)

        input_prompts = [self.prompt_template.get_string(question=q) for q in input_query]
...
class ConditionalPipeline(BasicPipeline):
    def __init__(self, config, prompt_template=None, conv_prompt=None):
        ...
        self.sequential_pipeline = SequentialPipeline(
            config, prompt_template, retriever=self.retriever, generator=self.generator, conv_prompt=self.conv_prompt
        )
        ...
        self.zero_shot_templete = PromptTemplate(
            ...
            system_prompt="根据自己的知识回答问题。只给我答案，不要输出任何其他的话。",
