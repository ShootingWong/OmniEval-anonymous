...
class Index_Builder:
    ...
    def __init__(
        ...
        print(f'self.corpus_path = {self.corpus_path}')
    ...
    def st_encode_all(self):
        ...
        all_embeddings = []
        for i in tqdm(range(0, len(sentence_list), self.batch_size)):
            batch_data = sentence_list[i : i + self.batch_size]
    ...
    def build_dense_index(self):
        ...
        """
        if self.use_fp16:
            loguru.logger.info("Using FP16 for sentence transformer model.")
        else:
            loguru.logger.info("Using FP32 for sentence transformer model.")

        if os.path.exists(self.index_save_path):
            print("The index file already exists and will be overwritten.")

        if self.use_api_encoder:
            from utils.api_generator import APIEncoder
            self.encoder = APIEncoder(self.retrieval_method)
            ...
            loguru.logger.info(f"model_path: {self.model_path}")
            hidden_size = self.encoder.config.hidden_size
            loguru.logger.info(f"hidden_size: {hidden_size}")
