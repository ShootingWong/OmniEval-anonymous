...
from flashrag.retriever.encoder import Encoder, STEncoder, FEEncoder
...
class DenseRetriever(BaseRetriever):
    ...
    def __init__(self, config: dict):
        ...
        self.corpus = load_corpus(self.corpus_path)
        if config["retrieval_use_fp16"]:
            loguru.logger.info("Using float16 for retrieval")
        else:
            loguru.logger.info("Using float32 for retrieval")
        ...
        elif config["use_flag_embedding"]:
            self.encoder = FEEncoder(
                model_name=self.retrieval_method,
                model_path=config["retrieval_model_path"],
