...
def get_generator(config, **params):
    ...
    elif config["framework"] == "vllm-api":
        return getattr(importlib.import_module("utils.api_generator"), "VLLMAPIGenerator")(config, **params)
    elif config["framework"] == "vllm-openai":
        return getattr(importlib.import_module("utils.api_generator"), "VLLMOpenAIGenerator")(config, **params)
    elif config["framework"] == "tgi-api":
        return getattr(importlib.import_module("utils.api_generator"), "TGIAPIGenerator")(config, **params)
...
def get_retriever(config):
    ...
    if config["retrieval_method"] == "bm25":
        ...
        print(f"use_sentence_embedding: {config['use_sentence_transformer']}, use_flag_embedding: {config['use_flag_embedding']}")
